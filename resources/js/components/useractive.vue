...
export default {
    ...
    methods:{
            mountedFunction(){
                ...
                axios.get('/api/view-user-pickup-requests').then(response=>{
                    ...
                    $.each(this.itemsa, function(key,value){
                        ...
                        this.itemsa[key].docs=JSON.parse(value.docs);
                            ...
                            axios.get('/api/view-user-delivery-requests').then(response=>{
            ...
            userUpdateItems(data){
                var ids=data.id;
                var delivery_id=ids.substring(4);
                var keys=data.keys;
                var docs=data.docs;
                var user_id = this.user_id;
                var stripe=data.customer_id_stripe;
                console.log(keys);
                console.log(docs);
                axios.post('/api/user-update-items',{
                    delivery_id,
                    keys,
                    docs,
                    user_id,
                }).then(response=>{
                    var id=response.data.delivery_id;
