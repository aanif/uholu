...
<b-container fluid>
    ...
    >
      <template v-slot:cell(user_id)="row" >
        <b v-if="row.item.flag==0">{{ row.item.user_id }}</b>
        <span v-if="row.item.flag==1">{{ row.item.user_id }}</span>
      </template>
      <template v-slot:cell(user_name)="row" >
        <b v-if="row.item.flag==0">{{ row.item.user_name }}</b>
        <span v-if="row.item.flag==1">{{ row.item.user_name }}</span>
      </template>
      <template v-slot:cell(subject)="row" >
        <div style="white-space: nowrap; width: 350px; overflow: hidden;text-overflow: ellipsis"><b v-if="row.item.flag==0">{{row.item.subject}}</b></div>
        <div style="white-space: nowrap; width: 350px; overflow: hidden;text-overflow: ellipsis"><span v-if="row.item.flag==1">{{ row.item.subject }}</span></div>
      </template>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Subject</label>
                                                    <input type="text" class="form-control" disabled="" name="zip" placeholder="ZIP Code" v-model="row.item.user_email">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label>Subject</label>
                                                    <input type="text" class="form-control" disabled="" name="zip" placeholder="ZIP Code" v-model="row.item.subject">
                                                </div>
                                            </div>
                                        </div>
                                        <div class="row">
...
export default {
    ...
    data(){
            ...
            return{
                ...
                fields: [
                { key: 'user_id', label: 'Customer ID', sortable: true, sortDirection: 'desc' },
                ...
                { key: 'subject', label: 'Subject', sortable: true, sortDirection: 'desc' },
    ...
    methods:{
          ...
          axios.get('/api/viewSupport/').then(response=>{
                ...
                $.each(this.items, function(key,value){
                     ...
                     if(this.items[key].user_role=='customer'){
                     this.items[key].user_id='1003'+value.user_id;}
                     else if(this.items[key].user_role=='business'){
            ...
            deleteMessage(data){
                var id=data.id;
                axios.delete('/api/SupportDelete/'+id).then(response=>{
